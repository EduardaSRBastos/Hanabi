...
int flag=-1;	
	do
		{int check=1;
			printf("Insira o número da ação que pretende fazer, %s:\n",jog1);
			printf("1-dar informação; 2-descartar carta; 3-jogar carta: ");
			check=scanf("%d",&acao);
			if(acao>3 || check==0)
			{
			...
			{
				printf("Não pode dar informação, pois já não tem pistas.\n\n");
				flag=0;
			}
			else if(acao==2 && pistas==8)
			{
				printf("Não pode descartar uma carta, pois tem todas as pistas.\n\n");
				flag=0;
			}
			else if(acao==2 && total==0)
			{
				printf("Não pode descartar uma carta, pois não pode buscar mais à compra.\n\n");
				flag=0;
			}
...
void menu(char jog1[17])
{
	...
	printf("Criado por: 70647- Eduarda Bastos | 70592- Jorge Cunha | 70650- Rui Duarte\n\n");
	...
	do
	{
		...
		printf("C. Apresentar uma descrição do jogo na consola\n"); 
		...
		if(ver=='C')
		{
			...
			printf("·Objetivo do jogo:\nHanabi é um jogo cooperativo, isso significa que os jogadores não competem uns contra os outros, mas sim trabalham em conjunto em busca de um objetivo.\nNeste jogo, os jogadores são fabricantes de fogos de artifício que, acidentalmente, misturaram suas pólvoras, corantes e foguetes.\nO festival vai começar e o pânico toma conta de todos. Eles tem que trabalhar juntos para impedir que o show seja um desastre!\nOs pirotécnicos devem juntar as 5 cores de fogos de artifício diferentes (branco, vermelho, azul, amarelo e verde),\ncolocando-os em série (1, 2, 3, 4 e 5) dentro de suas cores.\n\n");
			printf("·Preparação:\nSepare as 5 cartas multicolores, elas serão usadas somente para a expansão.\nColoque as 8 cartas de dica ao lado da pilha de compras e as 3 cartas de penalidade em baixo da caixa do jogo.\nEmbaralhe as 50 cartas Hanabi e coloque-as viradas para baixo, formando uma pilha de compras. Distribua as 5 cartas para cada jogador.\n\n");
			printf("·Importante:\nOs jogadores não devem olhar as cartas que recebem!\nCada um pega suas próprias cartas de uma maneira que os outros jogadores conseguem ver a carta, mas ele próprio não consegue!\nOs jogadores não podem olhar suas próprias cartas durante todo o jogo. Isso significaria uma desonra.\n\n");
			printf("·O Jogo:\nO jogador com as roupas mais coloridas deve começar o jogo. Os jogadores então fazem sua jogada e o jogo sempre segue no sentido horário.\nEm seu turno, um jogador deve realizar uma, e somente uma das três ações abaixo (você não pode pular sua vez):\n-Dar uma informação.\n-Descartar uma carta.\n-Jogar uma carta.\n\nNota : Quando é a vez de um jogador, seus colegas não podem comentar nada ou tentar influenciá-lo.\n\n");
			printf("·Dar uma informação:\nPara realizar esta ação, o jogador deve retornar uma carta de dica para dentro da caixa do jogo.\nAgora ele pode passar uma informação a um dos colegas, sobre as cartas que estão na mão daquele colega.\nImportante:\nO jogador deve apontar claramente para a carta sobre a qual está dando informação.\nDois tipos de informações podem ser passados:\n-Informações sobre uma cor (somente uma).\n-Informações sobre um número (somente um).\nImportante:\nO jogador deve dar uma informação completa: Se o seu colega possui duas cartas verdes, o jogador não pode apontar somente uma delas!\nNota:\nEsta ação só pode ser realizada se existirem cartas de dica disponíveis ao lado da pilha de compras.\nSe todas as cartas de dica estiverem dentro da caixa do jogo, o jogador deve escolher outra ação.\n\n");
			printf("·Descartar uma carta:\nPara realizar essa ação, o jogador deve tirar uma carta de dica de dentro da caixa do jogo e colocá-la ao lado da pilha de compras.\nEntão, o jogador descarta uma carta de sua mão e a coloca na pilha de descartes (ao lado da pilha de compras, com a face virada para cima).\nDepois o jogador compra uma nova carta da pilha de compras e a coloca em sua mão, sem ver seu conteúdo!\nNota:\nEsta ação só pode ser realizada se existirem cartas de dica dentro da caixa do jogo.\nSe todas as cartas de dica estiverem ao lado da pilha de compras, o jogador deve escolher outra ação.\n\n");
			printf("·Jogar uma carta:\nO jogador pega uma carta de sua mão e a coloca em sua frente.\nDuas opções são possíveis:\nA carta inicia ou continua uma sequência de fogos, e então é adicionada aos fogos.\nA carta revelada não se encaixa em nenhuma sequência de fogos:\nEla é descartada e uma carta de penalidade (com o foguete) é retirada debaixo da caixa do jogo.\n\n\n");
